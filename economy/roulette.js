  ...
  let prefix = bot_prefix;
    ...
    if (random == 0 && colour == 2) { // Hijau
      ...
      bal[message.author.id].balance = curBal1 + money;
        ...
        message.channel.send(`**${message.author.username}**, 💚 **JACKPOT** You won <:dollar:705795560363524166> **${money}** 💚 | The Number was **${random}**`);
      ...
      bal[message.author.id].balance = curBal2 + money
        fs.writeFile('./src/database/balance.json', JSON.stringify(bal, null, 2), (err) => {
        message.channel.send(`**${message.author.username}**, 🔴 You won <:dollar:705795560363524166> **${money}** 🔴 | The Number was **${random}**`);
          if(err) console.log(err) 
        });
      ...
      let curBal4 = bal[message.author.id].balance
        ...
        fs.writeFile('./src/database/balance.json', JSON.stringify(bal, null, 2), (err) => {
        message.channel.send(`**${message.author.username}**, You sadly lost <:dollar:705795560363524166> **${money}** | The Number was **${random}**`);
          if(err) console.log(err) 
        });
    }
