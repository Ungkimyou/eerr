...
exports.run = async (client, message, args, color) => {
  ...
  if (bal[message.author.id].balance < amount) return message.channel.send(`**${message.author.username}**, You not have insufficient balance yet you need <:dollar:705795560363524166> **${amount}** for changes background, Keep active and don't forget to take your daily everyday!`) 
  ...
  fs.writeFile('./src/database/background.json', JSON.stringify(bg, null, 2), (err) => {
    ...
    let bgEmbed = new Discord.RichEmbed() 
      ...
      .setDescription(`<:dollar:705795560363524166> **${amount}** has been deducted from your balance for change the profile card background!`)
