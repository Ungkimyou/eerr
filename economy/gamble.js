  ...
  let prefix = bot_prefix;
    ...
    if (random == 0) { // Jackpot
      ...
      bal[message.author.id].balance = curBal1 + money;
        ...
        message.channel.send(`🎲 | **${message.author}**, 🎉🎉 **JACKPOT** You won <:dollar:705795560363524166> **${money}** 🎉🎉 Wow Congrats 🎉`);
      ...
      bal[message.author.id].balance = curBal2 + money
        fs.writeFile('./src/database/balance.json', JSON.stringify(bal, null, 2), (err) => {
        message.channel.send(`🎲 | Congrats **${message.author.username}**, 🎉 You won <:dollar:705795560363524166> **${money}** 🎉 and got keep what you had.`);
          if(err) console.log(err) 
        });
      ...
      let curBal5 = bal[message.author.id].balance
        ...
        fs.writeFile('./src/database/balance.json', JSON.stringify(bal, null, 2), (err) => {
        message.channel.send(`🎲 | **${message.author.username}**, You sadly lost <:dollar:705795560363524166> **${money}**, I hope you do better next time 😦`);
          if(err) console.log(err) 
        });
    }
