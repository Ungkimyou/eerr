...
const decks = new Map();
  ...
  let amount = parseInt(args[0]);
    ...
    if (amount > 99990000) return message.channel.send('Bet msx to 999 only');
    ...
    let curBal = bal[message.author.id].balance;
		...
		try {
			...
			if (dealerInitialTotal === 21 && playerInitialTotal === 21) {
				...
				return message.channel.send(`Ouch, the dealer hit blackjack right away! You lost <:dollar:705795560363524166> **${amount}**, I hope you do better next time!`);
				});
                ...
                fs.writeFile('./src/database/balance.json', JSON.stringify(bal, null, 2), (err) => {
                 if(err) console.log(err);
				return message.channel.send(`Wow, you hit blackjack right away! Lucky and you won <:dollar:705795560363524166> **${amount}**!`);
				});
			}
			let playerTurn = true;
			let win = false;
			...
			bal[message.author.id].balance = curBal + amount;
                fs.writeFile('./src/database/balance.json', JSON.stringify(bal, null, 2), (err) => {
                 if(err) console.log(err);
                 return message.channel.send(`${reason}! You won <:dollar:705795560363524166> **${amount}**!`);
			});
			} else {
			bal[message.author.id].balance = curBal - amount;
                fs.writeFile('./src/database/balance.json', JSON.stringify(bal, null, 2), (err) => {
                 if(err) console.log(err);
			     return message.channel.send(`${reason}! You lost <:dollar:705795560363524166> **${amount}**.`);
			});
			} 
		} catch (err) {
			decks.delete(message.channel.id);
