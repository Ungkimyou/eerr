...
exports.run = async (client, message, args, color) => {
  ...
  if(!ball[message.author.id]){
   ...
   if(ball[message.author.id].balance < args[1]) return message.channel.send(`Check again **${message.author.username}**, You dont have \<:dollar:705795560363524166>**${args[1]}**`);
  ...
  fs.writeFile('./src/database/balance.json', JSON.stringify(bal, null, 2), (err) => {
    user.send(`\🏧  | **Transfer Receipt**\`\`\`You have received <:dollar:705795560363524166> ${args[1]} from user ${message.author.tag}\n(ID: ${message.author.id})\`\`\``);
  ...
  fs.writeFile('./src/database/balance.json', JSON.stringify(ball, null, 2), (err) => {
    message.channel.send(`Hey **${user.tag}**, You got \<:dollar:705795560363524166> **${args[1]}** from **${message.author.tag}**`);
    if(err) console.log(err);
  });
