...
module.exports = async (client, member) => {
   ...
   var leave = JSON.parse(fs.readFileSync("./src/json/leave.json", "utf8"));
    let leavesetting = JSON.parse(
      fs.readFileSync("./src/json/leaveonoff.json", "utf8")
    );
    if (!leavesetting[member.guild.id]) {
      leavesetting[member.guild.id] = {
        values: 1
      };
    }
    if (!leave[member.guild.id]) return;
    let values = leavesetting[member.guild.id].checker;
    ...
    if (values === 1) {
      var leave = JSON.parse(fs.readFileSync("./src/json/leave.json", "utf8"));
      if (!leave) return;
      let channel = member.guild.channels.get(`${leave[member.guild.id].nick}`);
      if (!channel) return;
      ...
      ctx.fillText("Goodbye,", canvas.width / 3.2, canvas.height / 3.5);
